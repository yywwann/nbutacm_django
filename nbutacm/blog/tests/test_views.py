...
class BlogDataTestCase(TestCase):
    def setUp(self):
        ...
        self.post1 = Post.objects.create(
            title="测试标题一", body="测试内容一", category=self.cate1,
            # author=self.user,
        )
...
class PostDetailViewTestCase(BlogDataTestCase):
    def setUp(self):
        ...
        self.md_post = Post.objects.create(
            title="Markdown 测试标题", body="# 标题", category=self.cate1,
            # author=self.user,
        )
    ...
    def test_good_view(self):
        ...
        # self.assertContains(response, self.md_post.title)  # 是否包含标题
    ...
    #     response = self.client.get(self.url)
    #     self.assertContains(response, "文章目录")  # 是否包含 TOC
